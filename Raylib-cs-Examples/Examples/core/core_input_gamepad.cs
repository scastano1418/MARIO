...
using static Raylib_cs.GamepadNumber;
...
namespace Examples
{
    public class core_input_gamepad
    {
            ...
            while (!WindowShouldClose())    // Detect window close button or ESC key
            {
                ...
                if (IsGamepadAvailable(GAMEPAD_PLAYER1))
                {
                    ...
                    if (IsGamepadName(GAMEPAD_PLAYER1, XBOX360_NAME_ID))
                    {
                        ...
                        DrawCircle(259 + (int)(GetGamepadAxisMovement(GAMEPAD_PLAYER1, GAMEPAD_AXIS_LEFT_X) * 20),
                                   152 - (int)(GetGamepadAxisMovement(GAMEPAD_PLAYER1, GAMEPAD_AXIS_LEFT_Y) * 20), 25, BLACK);

                        // Draw axis: right joystick
                        DrawCircle(461, 237, 38, BLACK);
                        DrawCircle(461, 237, 33, LIGHTGRAY);
                        DrawCircle(461 + (int)(GetGamepadAxisMovement(GAMEPAD_PLAYER1, GAMEPAD_AXIS_LEFT_X) * 20),
                                   237 - (int)(GetGamepadAxisMovement(GAMEPAD_PLAYER1, GAMEPAD_AXIS_LEFT_Y) * 20), 25, BLACK);

                        // Draw axis: left-right triggers
                        DrawRectangle(170, 30, 15, 70, GRAY);
                        DrawRectangle(604, 30, 15, 70, GRAY);
                        DrawRectangle(170, 30, 15, (int)(((1.0f + GetGamepadAxisMovement(GAMEPAD_PLAYER1, GAMEPAD_AXIS_LEFT_TRIGGER)) / 2.0f) * 70), RED);
                        DrawRectangle(604, 30, 15, (int)(((1.0f + GetGamepadAxisMovement(GAMEPAD_PLAYER1, GAMEPAD_AXIS_RIGHT_TRIGGER)) / 2.0f) * 70), RED);
                    ...
                    else if (IsGamepadName(GAMEPAD_PLAYER1, PS3_NAME_ID))
                    {
                        ...
                        if (IsGamepadButtonDown(GAMEPAD_PLAYER1, GAMEPAD_BUTTON_MIDDLE_RIGHT)) DrawTriangle(new Vector2(436, 168), new Vector2(436, 185), new Vector2(
                        464, 177), RED);
                        ...
                        DrawCircle(319 + (int)(GetGamepadAxisMovement(GAMEPAD_PLAYER1, GAMEPAD_AXIS_LEFT_X) * 20),
                                   255 + (int)(GetGamepadAxisMovement(GAMEPAD_PLAYER1, GAMEPAD_AXIS_LEFT_Y) * 20), 25, BLACK);

                        // Draw axis: right joystick
                        DrawCircle(475, 255, 35, BLACK);
                        DrawCircle(475, 255, 31, LIGHTGRAY);
                        DrawCircle(475 + (int)(GetGamepadAxisMovement(GAMEPAD_PLAYER1, GAMEPAD_AXIS_RIGHT_X) * 20),
                                   255 + (int)(GetGamepadAxisMovement(GAMEPAD_PLAYER1, GAMEPAD_AXIS_RIGHT_Y) * 20), 25, BLACK);

                        // Draw axis: left-right triggers
                        DrawRectangle(169, 48, 15, 70, GRAY);
                        DrawRectangle(611, 48, 15, 70, GRAY);
                        DrawRectangle(169, 48, 15, (int)(((1.0f - GetGamepadAxisMovement(GAMEPAD_PLAYER1, GAMEPAD_AXIS_LEFT_TRIGGER)) / 2.0f) * 70), RED);
                        DrawRectangle(611, 48, 15, (int)(((1.0f - GetGamepadAxisMovement(GAMEPAD_PLAYER1, GAMEPAD_AXIS_RIGHT_TRIGGER)) / 2.0f) * 70), RED);
                    }
                    else
                    {
                    ...
                    {
                        DrawText(string.Format("AXIS {0}: {0:0.02}", i, GetGamepadAxisMovement(GAMEPAD_PLAYER1, (GamepadAxis)i)), 20, 70 + 20 * i, 10, DARKGRAY);
                    }
